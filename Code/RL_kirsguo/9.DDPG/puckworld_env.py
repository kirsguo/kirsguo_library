...
class PuckWorldEnv(gym.Env):
    ...
    def _clip(self, x, min_speed, max_speed):
        if x < min_speed:
            return min_speed
        elif x > max_speed:
            return max_speed
    ...
    def _compute_dis(self, distance_x, distance_y):
        return math.sqrt(math.pow(distance_x, 2) + math.pow(distance_y, 2))

    def step(self, action):
        self.action = action
        # 获取agent位置，速度，目标位置
        ppx, ppy, pvx, pvy, target_x, target_y = self.state
        ppx, ppy = ppx + pvx, ppy + pvy  # 更新位置
        pvx, pvy = pvx * 0.95, pvy * 0.95  # 摩擦导致速度损失
        pvx += self.accelerate * action[0]  # 横向 速度变化
        pvy += self.accelerate * action[1]  # 纵向 速度变化

        #  限速
        pvx = self._clip(pvx, -self.max_speed, self.max_speed)
        pvy = self._clip(pvy, -self.max_speed, self.max_speed)
        #  是否碰壁 如果碰壁 速度方向改变，大小损失为原来的一半
        if ppx < self.radius:              # left bound
            pvx *= -0.5
            ppx = self.radius
        if ppx > 1 - self.radius:          # right bound
            pvx *= -0.5
            ppx = 1 - self.radius
        if ppy < self.radius:              # bottom bound
            pvy *= -0.5
            ppy = self.radius
        if ppy > 1 - self.radius:          # right bound
            pvy *= -0.5
            ppy = 1 - self.radius

        self.t += 1
        if self.t % self.update_time == 0:  # update target position
            target_x = self._random_pos()         # randomly
            target_y = self._random_pos()

        distance_x, distance_y = ppx - target_x, ppy - target_y
        distance = self._compute_dis(distance_x, distance_y)  # agent to target

        self.reward = self.goal_dis - distance
        done = bool(distance <= self.goal_dis)
        self.state = (ppx, ppy, pvx, pvy, target_x, target_y)
